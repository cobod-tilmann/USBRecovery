...
# Manifest format is sha256sum-compatible text, signed with minisign.
...
Options:
  ...
  --force                Overwrite existing manifest/signature files
...
main() {
  ...
  if [[ "$interactive" == "true" ]]; then
    note "=== Signed Manifest Generator ==="
  ...
  local script_path manifest_path sig_path modules_dir_rel modules_dir_abs
  ...
  if [[ ( -e "$manifest_path" || -e "$sig_path" ) && "$FORCE" != "true" ]]; then
    if [[ "$interactive" == "true" ]]; then
      if ! confirm "Manifest/signature already exists. Overwrite"; then
        note "Aborted."
        exit 0
      fi
    else
      err "Manifest/signature exists. Use --force to overwrite."
      exit 1
    fi
  fi
